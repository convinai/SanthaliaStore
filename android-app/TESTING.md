...
├── ResumeThresholdTest.kt   # Pins the 5-min activity-resume auto-sync rule
├── util/
...
| No background sync | nothing is enqueued via WorkManager any more — no periodic / scheduled sync exists. `notifyChange` only fires while the process is alive (debounced auto-sync); the moment the app is killed, zero work happens. | Manual smoke ("No background drain" below) |
| Activity-triggered auto-sync | App start / resume after ≥ 5 min away kicks off `runAutoSync` silently; resume within 5 min is a no-op | ResumeThresholdTest + Manual smoke ("Auto-sync on cold start" / "Auto-sync on resume…" below) |
| Activity-triggered auto-sync | Local save / delete pings a debounced (`AUTO_SYNC_DEBOUNCE_MS = 1500 ms`) trigger; a burst of N writes coalesces into 1 sync | Manual smoke ("Auto-sync on write" below) |
| Activity-triggered auto-sync | `runAutoSync` silently skips when `sheetUrl` is blank (no `lastSyncError`, no log spam, no snackbar) | Manual smoke ("Auto-sync silent when URL not configured" below) |
| Activity-triggered auto-sync | Auto-sync errors land in `lastSyncError` for Settings → Sync details, but NEVER produce a snackbar — only manual sync user-faces errors | Manual smoke ("Auto-sync silent on error" below) |
| Single-flight sync | `syncMutex` prevents `runFullSyncNow` and `runAutoSync` from overlapping; auto-sync uses `tryLock` (drops overlapping requests), manual uses `withLock` (waits for any in-flight auto-sync) | Manual smoke ("Manual tap mid-auto-sync still runs" below) |
| Single source of truth for syncing | `SyncRepository.isSyncing` flips for ALL sync flavours (auto + manual); both Home and Settings observe this same flow | Manual smoke ("Sync ho raha hai indicator covers auto-sync" below) |
...
| Home screen | "Last sync" line under the search bar shows relative time and "Abhi tak sync nahi hua" pre-first-sync; flips to "Sync ho raha hai…" while ANY sync (auto or manual) is in progress | Manual smoke ("Home last-sync label" / "Sync ho raha hai indicator covers auto-sync" below) |
| Settings screen | "Last sync" line on the URL summary card mirrors Home — flips to "Sync ho raha hai…" during auto-sync, not just manual sync | Manual smoke ("Sync ho raha hai indicator covers auto-sync" below) |
...
- [ ] **Save button location** — On Add-item, Edit-item, Add-entry, Edit-entry, the Save action ("Save karein") sits in the TopAppBar's right-hand `actions` slot, NOT at the bottom of the form. The form ends at the last input (no stray 32 dp gap). Tapping Save runs validation, persists, and pops the screen the same way the old bottom button did.
- [ ] **Save button disabled state** — On any of the four forms above, tap Save once. While `isSaving` is true the TopAppBar Save reads as disabled (greyed out, tap is a no-op) so a second tap can't double-write. The TopAppBar layout itself does NOT shift between enabled/disabled states.
- [ ] **TextField visible while typing — Add-entry** — Open Add-entry on a small phone (≤ 5"). Tap into the Notes field at the bottom of the form, type a couple of words. The Notes field stays visible above the soft keyboard the entire time (it does NOT get hidden behind the IME). Repeat for Supplier and Quantity.
- [ ] **TextField visible while typing — Add-item with first purchase fields** — Open Add-item, fill code + name, scroll down to the optional "Aaj ka rate" section, then tap each field (Price → Quantity → Supplier → Notes) in turn. Every field stays visible above the keyboard as you type; you can read what you typed without dismissing the IME.
- [ ] **TextField visible while typing — Settings URL editor** — Open Settings, tap the pencil to edit the Sheet URL, focus the URL field and type. The field stays visible above the keyboard.
...
- [ ] **Auto-sync on cold start** — kill the app from recents (`adb shell am force-stop in.santhaliastore.ratecard.debug`), then reopen. The "Sync ho raha hai…" line briefly appears under the search bar (and the refresh icon swaps to a spinner) within ~1 s of the home list painting; once the round-trip completes, the line flips to "Last sync: \<now\>" and any new server-side rows show up in the list. No snackbar fires.
- [ ] **Auto-sync on resume after 5+ min away** — open the app, then background it (Home gesture / lock screen) for at least 6 minutes. Foreground it again → the "Sync ho raha hai…" line appears briefly, then "Last sync: \<now\>" updates. Confirms the `ProcessLifecycleOwner` `ON_START` observer fires only after the threshold, and `runAutoSync` runs silently.
- [ ] **Auto-sync on resume within 5 min is a no-op** — open the app, background it for ~30 seconds, foreground. The "Sync ho raha hai…" line should NOT appear; `lastSyncedAt` should NOT advance. (Saves bandwidth on every quick task switch.)
- [ ] **Auto-sync on write** — with the URL configured and the app already foreground, add or edit an item without tapping any sync button. Within ~1.5 s of saving (the debounce window) the "Sync ho raha hai…" line briefly flashes, then "Last sync: \<now\>" updates and the new row is visible on the sheet. Repeat with 5 quick saves in a single second → only one sync fires (the debouncer collapses the burst).
- [ ] **Auto-sync silent on error** — turn off Wi-Fi (and mobile data), then add an item. The debounced auto-sync runs, the indicator briefly shows, and the failure lands in Settings → "Pichhli error" / Sync details — but NO snackbar appears on Home or Settings. Re-enable network and tap manual refresh: the snackbar then surfaces the success outcome as usual.
- [ ] **Auto-sync silent when URL not configured** — fresh install with no Sheet URL. Open / resume the app several times, add an item, edit it. No "Sync ho raha hai…" indicator should appear (`runAutoSync` skips silently because `sheetUrl` is blank), no entries should land in `lastSyncError`, no log spam in `adb logcat | grep -i sync`. The pending count grows as the user edits — those will be pushed the moment a URL is set and the next auto-sync trigger fires.
- [ ] **Manual tap mid-auto-sync still runs** — kick off an auto-sync (cold start or post-write debounce). Before it completes, tap the Home refresh button. The button is greyed out while the auto-sync runs (single source of truth — `SyncRepository.isSyncing`), and the moment the auto-sync finishes the manual tap immediately runs through (`syncMutex.withLock` queues the manual after the auto). The manual sync's snackbar fires as today.
- [ ] **Sync ho raha hai indicator covers auto-sync** — during an auto-sync (resume-trigger OR post-write), the "Last sync" line on Home AND on Settings should both flip to "Sync ho raha hai…" simultaneously, and the Home top-bar refresh icon should swap to a spinner. When the sync completes, all three indicators clear together (single source of truth — both screens read `SyncRepository.isSyncing`).
- [ ] **No background drain** — leave the app fully closed (kill via `am force-stop`) for 30+ minutes. Open `adb shell dumpsys batterystats --reset && adb shell dumpsys batterystats | grep santhaliastore` after the wait → the package should show effectively zero CPU / network usage during the closed window. Confirms there's no WorkManager / scheduled job we accidentally left in.
